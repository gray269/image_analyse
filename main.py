...
class DefectCompareApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Dossier TXT", command=self.pick_text_folder).grid(row=0, column=0, padx=4, pady=4)
        ttk.Entry(top, textvariable=self.text_dir_var, width=78).grid(row=0, column=1, padx=4, pady=4, sticky="ew")

        ttk.Button(top, text="Dossier images", command=self.pick_image_folder).grid(row=1, column=0, padx=4, pady=4)
    ...
    def pick_text_folder(self):
        folder = filedialog.askdirectory(title="Choisir le dossier des fichiers TXT")
        if folder:
            self.text_dir_var.set(folder)

    def pick_image_folder(self):
        folder = filedialog.askdirectory(title="Choisir le dossier racine des images")
        if folder:
